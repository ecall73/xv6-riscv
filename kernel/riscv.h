...
r_stimecmp()
{
  uint32 hi, lo, hi2;
  do {
    asm volatile("csrr %0, 0x15d" : "=r"(hi));
    asm volatile("csrr %0, 0x14d" : "=r"(lo));
    asm volatile("csrr %0, 0x15d" : "=r"(hi2));
  } while (hi != hi2);
...
w_stimecmp(uint64 x)
{
  asm volatile("csrw 0x15d, %0" : : "r"(0xffffffffU));
  asm volatile("csrw 0x14d, %0" : : "r"((uint32)x));
  asm volatile("csrw 0x15d, %0" : : "r"((uint32)(x >> 32)));
}
...
// Sv32, leaving the upper half for the kernel.
#define MAXVA (1UL << (10 + 10 + 12 - 1))
